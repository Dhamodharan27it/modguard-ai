...
forms.post('/mop-comment-submit', async (c) => {
  ...
  if (!values.remove && !values.lock) {
    return c.json<UiResponse>(
      { showToast: 'Please select either lock or remove.' }, 200
    );
  }
  ...
  if (!isT1(targetId)) {
    return c.json<UiResponse>(
      { showToast: 'Action failed! Invalid comment ID.' }, 200
    );
  }
  ...
  try {
    ...
    if (values.remove) {
      ...
      const { record, banInfo } = await addStrike(
        author, reason, analysis.category, targetId
      );
      ...
      await logAction({
        itemId: targetId, type: 'comment', action: 'removed',
        author, reason, violation: analysis.violation,
        strikeCount: record.count, banApplied: banInfo.label,
        permanent: banInfo.permanent, auto: false,
      });
    }
    ...
    if (values.lock) {
      await comment.lock();
      await logAction({
        itemId: targetId, type: 'comment',
        action: 'locked', author, reason, auto: false,
      });
    }
    ...
    return c.json<UiResponse>(
      { showToast: '❌ Action failed. Please try again.' }, 200
    );
  }
});
...
forms.post('/mop-post-submit', async (c) => {
  ...
  if (!isT3(targetId)) {
    return c.json<UiResponse>(
      { showToast: 'Action failed! Invalid post ID.' }, 200
    );
  }
  ...
  try {
    ...
    if (values.remove) {
      ...
      await logAction({
        itemId: targetId, type: 'post', action: 'removed',
        author, reason, violation: analysis.violation,
        strikeCount: record.count, banApplied: banInfo.label,
        permanent: banInfo.permanent, auto: false,
      });
